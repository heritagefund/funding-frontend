...
RSpec.describe PreApplication::ExpressionOfInterest::HeritageFocusController do
  ...
  describe 'PUT #update' do
    ...
    it 'should re-render the page if a validation error occurs' do
      ...
      put :update, params: {
        pre_application_id: pre_application.id,
        pa_expression_of_interest: { heritage_focus: 'word ' * 101 }
      }

      expect(
        assigns(:pre_application).pa_expression_of_interest.errors[:heritage_focus]
      ).to include(
        I18n.t(
          'activerecord.errors.models.pa_expression_of_interest.attributes.heritage_focus.too_long',
          word_count: 100
        )
      )
    ...
    it 'should successfully progress if an empty heritage_focus ' \
      'param is passed' do

      put :update, params: {
        pre_application_id: pre_application.id,
        pa_expression_of_interest: { heritage_focus: '' }
      }
      ...
      expect(response).to(
        redirect_to(
          :pre_application_expression_of_interest_what_is_the_need_for_this_project
        )
      )
      ...
      expect(
        assigns(:pre_application).pa_expression_of_interest.heritage_focus
      ).to eq(nil)

    end

    it 'should successfully update if a populated heritage_focus ' \
      'param is passed' do

      put :update, params: {
        pre_application_id: pre_application.id,
        pa_expression_of_interest: {
            heritage_focus: 'Test value'
        }
      }
