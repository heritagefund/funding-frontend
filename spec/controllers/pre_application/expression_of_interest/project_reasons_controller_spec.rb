...
RSpec.describe PreApplication::ExpressionOfInterest::ProjectReasonsController do
  ...
  describe 'PUT #update' do
    ...
    it 'should re-render the page if a validation error occurs' do
      ...
      put :update, params: {
        pre_application_id: pre_application.id,
        pa_expression_of_interest: { project_reasons: 'word ' * 201 }
      }

      expect(
        assigns(:pre_application).pa_expression_of_interest.errors[:project_reasons]
      ).to include(
        I18n.t(
          'activerecord.errors.models.pa_expression_of_interest.attributes.project_reasons.too_long',
          word_count: 200
        )
      )
    ...
    it 'should successfully progress if an empty project_reasons ' \
      'param is passed' do

      put :update, params: {
        pre_application_id: pre_application.id,
        pa_expression_of_interest: { project_reasons: '' }
      }
      ...
      expect(response).to(
        redirect_to(
          :pre_application_expression_of_interest_timescales
        )
      )
      ...
      expect(
        assigns(:pre_application).pa_expression_of_interest.project_reasons
      ).to eq(nil)

    end

    it 'should successfully update if a populated project_reasons ' \
      'param is passed' do

      put :update, params: {
        pre_application_id: pre_application.id,
        pa_expression_of_interest: {
            project_reasons: 'Test value'
        }
      }
